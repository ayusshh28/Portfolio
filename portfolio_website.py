...
# --- Page Config + Early Responsive CSS: this must be at the very top! ---
st.set_page_config(page_title="Ayush's Portfolio", layout="wide")

# --- Ultra-early strong mobile responsiveness CSS ---
st.markdown("""
<style>
...
@media (max-width: 600px) {
  ...
  .sidebar-tab { font-size: 15px !important; }
}

/* HOME headers and paddings */
@media (max-width: 600px) {
  .home-heading { font-size: 32px !important; }
  .home-container { padding: 14px 2vw !important; }
  .typing { font-size: 15px !important; }
}
/* IMAGES: never overflow */
.stImage img, .styled-image img {
  max-width: 96vw !important; width: 100% !important; height: auto !important; border-radius: 16px !important;
}
/* TABLES: scroll and shrink on mobile */
@media (max-width:600px) {
  table, th, td { font-size: 12px !important; padding: 6px !important; }
  table { display: block; overflow-x:auto;}
}
/* BUTTONS, TEXT FIELDS: fit phones */
@media (max-width:600px) {
  .stTextInput>div>input, .stTextArea>div>textarea, .stButton>button {
    font-size:15px !important; padding:6px 10px !important; max-width:98vw !important;
  }
}
</style>
""", unsafe_allow_html=True)

# SIDEBAR styling (keep the desktop widths only here—media-queries above override it for mobile)
st.markdown("""
<style>
section[data-testid="stSidebar"] {
...
.sidebar-title { font-size: 28px; font-weight: bold; text-align: center; margin-bottom: 30px;
  background: linear-gradient(90deg,#FFDE59,#FF914D); -webkit-background-clip:text; -webkit-text-fill-color:transparent;}
...
.main .block-container { max-width: 100% !important; padding: 1rem 3rem 2rem 3rem;}
.stTabs [role="tab"] { font-size: 18px; padding: 10px; background:#f0f0f0; color:#333;
  border-radius:10px 10px 0 0; margin-left:20px;}
.stTabs [role="tab"][aria-selected="true"] { background: #aed581; color: black;}
.stButton>button { background-color:rgba(44,83,100,0.4); color:white; border-radius:8px;}
</style>
""", unsafe_allow_html=True)
...
.typing { font-size:22px; color:black;margin-bottom:20px; border-right:3px solid #FFDE59;white-space:nowrap;overflow:hidden;width:0;animation:typing 3s steps(21, end) forwards;}
    ...
    with col1:
        ...
        with center:
            with open("resume/CV.pdf", "rb") as file:
                st.download_button(
                    label="📄 Download CV", data=file, file_name="CV_AyushKatiyar.pdf", mime="application/pdf", key="download_cv_button"
                )
    with col2:
...
# --- ABOUT ME ---
    ...
    st.markdown("""
<style>
.skill-tag {display:inline-block;padding:8px 16px;margin:6px;background-color:#fff;color:#333;border:1px solid #ccc;border-radius:10px;font-size:15px;font-family:'Segoe UI',sans-serif;}
table {width:100%;border-collapse:collapse;font-family:'Segoe UI',sans-serif;}
th, td {padding:12px;border:1px solid #ddd;text-align:center;font-size:15px;} th{background-color:#c4f2f6;color:#333;} td{background-color:#f1efff;color:#222;}
</style>
""", unsafe_allow_html=True)
    tab1, tab2, tab3 = st.tabs(["🎓 Education","💼 Skills","📜 Certifications"])
    with tab1:
        st.markdown("""
<h3 style='font-size:25px; font-weight:600;'>🎓 Education</h3><br>
<table>
<thead>
<tr>
<th>Qualification</th><th>Stream</th><th> Passout Year</th><th>Institute</th><th>City/State</th><th>Score</th>
</tr>
</thead>
<tbody>
<tr><td>BCA</td><td>-</td><td>2024</td><td>Babu Banarasi Das University</td><td>Lucknow, Uttar Pradesh</td><td>8.6 CGPA</td></tr>
<tr><td>12th</td><td>Science</td><td>2020</td><td>Wendy High School</td><td>Kanpur, Uttar Pradesh</td><td>82%</td></tr>
<tr><td>10th</td><td>-</td><td>2018</td><td>Wendy High School</td><td>Kanpur, Uttar Pradesh</td><td>87%</td></tr>
</tbody>
</table>
""", unsafe_allow_html=True)
    with tab2:
        ...
        st.markdown("""
<style>
.skills-container {display:flex;flex-wrap:wrap;gap:12px 14px;margin-top:15px;}
.skill-tag {padding:10px 18px; background-color: #fff; color:#333; border:1px solid #aaa; border-radius:10px; font-size:14px;font-weight:500;height:42px; box-shadow:1px 1px 4px rgba(0,0,0,0.08);}
</style>
""", unsafe_allow_html=True)
        skills = [
...
🔗 <b>GitHub Repository (Certificates Folder)</b>: <a href="https://github.com/ayusshh28/Certificates" target="_blank">View All Certificates</a>
</p>
""", unsafe_allow_html=True)
...
# --- PROJECTS ---
    ...
    with service_tabs[0]:
        ...
        with col2:
            st.markdown("""
    <div style="padding: 10px 20px; border-left: 6px solid #2563eb; background:#f1f5f9;border-radius:10px;margin-bottom:28px;">
      <div style="font-size:40px;font-weight:900;color:#1e3a8a;">LensX </div>
      <span style="font-size:18px;font-weight:500;color:#1e3a8a;"> (Lens Expert - NLP Suite) </span>
      ...
      <div style='font-size:14px; color:#CB4335; margin-top:12px;'><b>Note:</b> These AI models are trained on small sample datasets, intended for demo purposes—results are not 100% production-level.</div>
    </div>
            """, unsafe_allow_html=True)
        st.markdown("""
...
""", unsafe_allow_html=True)
    # === DASHBOARDS (repeat for your section as before, using st.image(..., use_column_width=True)) ===
...
# --- CONTACT ---
    ...
    with col1:
        st.markdown("""
<div style='padding:20px;background-color:#f9f9f9;border-radius:10px;margin-top:30px;box-shadow:0 4px 10px rgba(0,0,0,0.08);font-size:17px;line-height:2.7;'>
<p>
